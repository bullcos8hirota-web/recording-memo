...
## Phase 3: 要約エンジン統合
- [x] 要約モデル（`onnx-community/Qwen2.5-1.5B-Instruct`、`dtype: 'q4'`量子化）を暫定選定 — ⚠️ 複数モデル比較PoCは未実施（Phase 2のWhisperと同様の理由）。精度・速度に不満があれば`lib/summarize/config.ts`のモデルIDを差し替えて再評価する
- [x] `summarize.worker.ts` 実装（全文受信 → プロンプト構築 → 推論 → パース）
- [x] 出力フォーマット設計（概要／要点／決定事項／アクションアイテムのJSON、`SUMMARY_SYSTEM_PROMPT`で指示）
- [x] パース失敗時のフォールバック処理（生テキストを概要欄にそのまま表示、`parsed: false`をUIに警告表示）
- [x] `SummaryView`: 表示・編集・再実行ボタン
- [x] 文字起こし完了後に自動で要約をトリガーする連携処理（`RecordingItem.handleTranscribe`内）
- ⚠️ **実機での動作確認が未完了**: 検証環境（GPUなしヘッドレスブラウザ）ではWebGPUが使えずWASM実行になり、1.5Bモデルの生成に10分以上かかり検証を打ち切った。モデルDL（1.79GB、約80秒）は正常完了することは確認済み。実際にWebGPU対応のChrome/Edgeで生成が完了するか・実用的な速度か、次回ユーザー実機で確認が必要
