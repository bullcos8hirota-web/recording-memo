...
const SCHEDULE: { when: string; what: string; detail: string; where: string }[] = [
  {
    ...
    detail:
      '記録タブで手仕舞ったトレードを振り返り、監視タブのスコア順に候補を眺めます。買う場所(押し目・ブレイク)が近いものだけ残します。',
    where: 'アプリ: 記録 → 監視 → 銘柄',
  },
  {
    when: '日曜の夜',
  ...
  },
  {
    ...
    detail:
      '時系列ページから今週の5日分をコピーして貼り付けます。建玉タブで損切り・利確ラインとの距離を確認します。',
    where: 'アプリ: 取込 → 建玉',
  },
  {
    when: '土日',
    what: '結果を踏まえて来週を決める',
    detail:
      '手仕舞ったなら振り返りを書く。持ち越すなら建玉タブの「トレーリング目安」を見て、今の損切りより上ならそこまで引き上げ、SBI証券の逆指値も同じ値に出し直す。目安が今の損切りより下なら動かさない。次の候補を選び直す。',
    where: 'アプリ: 記録 → 建玉 → 監視',
  },
]
...
const ORDER_STEPS: { title: string; body: string; check?: string }[] = [
  {
    title: '1. 銘柄と株数を用意する',
    body: 'アプリの売買プランで出た「エントリー価格」「損切り価格」「株数」を控えます。株数は許容損失から逆算された数字で、自分の希望額ではありません。',
  },
  {
    title: '2. 買い注文を出す',
    body: '現物買いを選び、株数と価格(指値)を入力します。成行は避けます。想定より高く買うと、損切りまでの距離が変わって計算が崩れるためです。',
    check: '預り区分(特定口座 / NISA)を確認。NISAは損失を他の利益と相殺できません。',
  },
  {
    title: '3. 注文期間を決める',
    body: '「今週中」など、来週まで残らない期間にします。週明けに状況が変わっているのに、古い注文が生きているのは危険です。',
  },
  {
    title: '4. 約定したら、すぐ損切りの逆指値を出す',
    body: '「◯◯円以下になったら成行で売る」という売り注文を、アプリの損切り価格で入れます。ここが週次運用の生命線です。',
    check: '期間は可能な限り長く。期限切れで無防備になるのを防ぎます。',
  },
  {
    title: '5. 利確の注文をどうするか決める',
    body: 'SBI証券には、損切りと利確を同時に出せる注文方法があります(IFD・OCO・IFDOCOなど)。使えるなら、買いと同時に両方セットしてしまうのが確実です。使わない場合は、金曜の引け後に手動で判断します。',
    check: '注文方法の名前と使える条件は画面で確認してください。',
  },
]
