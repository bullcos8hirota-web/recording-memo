...
export function TradePlan({
  ...
  // 約定を受け取ったあとは、買い注文の案内を消す(二重に買わせないため)。
  const [filled, setFilled] = useState<number | null>(null)
  // 実際に買えた株数。約定価格がずれても、売る株数は持っている数のままでなければならない。
  const [filledShares, setFilledShares] = useState<number | null>(null)
  ...
  const showPlan =
    analysis === null || analysis.verdict === 'ready' || forceOrder
  ...
  // その価格で買ったときの損切り。ATRが出せない時も空欄にはしない。
  const stopFor = (entry: number) => {
    if (!snapshot || entry <= 0) return 0
    const atrStop =
      snapshot.atr14 !== null
        ? entry - snapshot.atr14 * settings.atrMultiple
        : null
    const fallback =
      snapshot.low5 !== null ? snapshot.low5 * 0.995 : entry * 0.95
    return Math.round(atrStop ?? fallback)
  }

  // 銘柄を切り替えたら、直近の高値の少し上とATR基準の損切りで引き直す。
  // 損切りは終値ではなく、この既定のエントリー価格から引く(でないと最初から食い違う)。
  // 引き直すのは銘柄か株価データが変わったときだけ。注文中フラグを消したような
  // 書き込みでも analysis は作り直されるので、それで入力を消してしまわないようにする。
  const initializedFor = useRef<string | null>(null)
  useEffect(() => {
    if (!snapshot) return
    const key = `${stock.code}@${snapshot.date}@${settings.atrMultiple}`
    ...
    setStopInput(String(stopFor(entry)))
    setForceOrder(false)
    setStopPinned(false)
    setFilled(null)
    setFilledShares(null)
    setSaved(null)
    // stopFor は snapshot と設定から作る純関数なので、依存に入れると毎回張り直しになる。
  ...
  useEffect(() => {
    if (stopPinned || !snapshot) return
    const typed = Number(entryInput)
    if (!typed) return
    const next = stopFor(typed)
    if (next > 0) setStopInput(String(next))
    // stopFor は snapshot と設定から作る純関数なので、依存に入れると毎回張り直しになる。
    // eslint-disable-next-line react-hooks/exhaustive-deps
  ...
  // 約定後は、株数は「買えた数」で固定する。約定価格から株数を計算し直すと、
  // 持っていない株数の売り注文を案内してしまう。
  const shares = filledShares ?? sizing.shares
  const grossProfit = plan.rewardPerShare * shares
  const exitFee = tradeFee(plan.target * shares, settings.feeConfig)
  const entryFee = tradeFee(plan.entry * shares, settings.feeConfig)
  const netProfit = grossProfit - entryFee - exitFee
  const afterTax = netProfit - capitalGainTax(netProfit)
  // 損切りが空欄・エントリー以上のままでは、売り注文の数字を出してはいけない。
  const stopValid = plan.stop > 0 && plan.stop < plan.entry
  const cost =
    filledShares === null ? sizing.cost : plan.entry * shares + entryFee
  const riskAmount =
    filledShares === null
      ? sizing.riskAmount
      : plan.riskPerShare * shares +
        entryFee +
        tradeFee(plan.stop * shares, settings.feeConfig)
  const riskRatio =
    filledShares === null
      ? sizing.riskRatio
      : (riskAmount / settings.capital) * 100

  const record = async () => {
    if (shares <= 0 || !stopValid) return
    await addTrade({
      code: stock.code,
      name: stock.name,
      entryDate: today(),
    ...
    setSaved(`${stock.name} ${shares}株の建玉を記録しました。`)
  }
    ...
    >
      {pending && (
        <PendingOrderCard
          stock={stock}
          onFilled={(fillPrice) => {
            setEntryInput(String(fillPrice))
            // 逆指値の約定価格は注文価格からずれる。損切りはその実際の価格から引き直す
            // (そうしないと、思っていたより大きなリスクのまま持つことになる)。
            setStopInput(String(stopFor(fillPrice)))
            setStopPinned(true)
            setForceOrder(true)
            setFilled(fillPrice)
            setFilledShares(pending.shares)
            setSaved(null)
            // 注文中の表示を消して、下に売り注文の数字を出す。ここが「約定した」の目に見える結果。
            void updateStock(stock.code, { pendingOrder: null })
          }}
        />
      )}
      ...
      {analysis && !ready && (
        <div className="mb-3 rounded-xl bg-amber-50 px-3 py-3 text-sm text-amber-900 dark:bg-amber-950/40 dark:text-amber-200">
          <p className="font-medium">
            今は買う場面ではありません（{VERDICT_LABEL[analysis.verdict]} /
            スコア{analysis.score}）
          </p>
      ...
      {showPlan && (
        <>
          ...
          {sizing.error && (filledShares === null || !stopValid) ? (
            ...
            <>
              <div className="mt-4 grid grid-cols-2 gap-2 sm:grid-cols-4">
                <Stat
                  ...
                  label="株数"
                  value={`${shares.toLocaleString('ja-JP')}株`}
                  hint={
                    filledShares !== null
                      ? '実際に買えた株数'
                ...
                <Stat
                  ...
                  label="必要資金"
                  value={yen(cost)}
                  hint={`資金の${((cost / settings.capital) * 100).toFixed(1)}%`}
                />
                <Stat
                  help="risk-per-trade"
                  label="想定損失"
                  value={yen(-riskAmount)}
                  hint={`資金の${riskRatio.toFixed(2)}%`}
          ...
          {earnings?.soon && (
            <div className="mt-4 rounded-xl bg-amber-50 px-3 py-3 text-sm text-amber-900 dark:bg-amber-950/40 dark:text-amber-200">
              <p className="font-medium">
                決算発表が{shortDate(stock.earningsDate!)}（あと{earnings.days}
                日）です
              </p>
              <p className="mt-1 text-xs">
          ...
          {exRights?.soon && (
            <p className="mt-4 rounded-xl bg-amber-50 px-3 py-2 text-sm text-amber-900 dark:bg-amber-950/40 dark:text-amber-200">
              権利確定日が{shortDate(stock.exRightsDate!)}（あと{exRights.days}
              日）です。 その翌営業日は配当の分だけ機械的に下がります。
            </p>
          )}
            ...
            <div className="mt-4 rounded-xl border border-neutral-200 p-3 dark:border-neutral-800">
              ...
              {filled === null && !ready && (
                <p className="mt-1 text-xs text-amber-700 dark:text-amber-300">
                  判定は「{analysis ? VERDICT_LABEL[analysis.verdict] : '—'}
                  」です。条件は揃っていません。
                </p>
              )}
              <ol className="mt-2 space-y-2 text-sm text-neutral-700 dark:text-neutral-300">
                {filled === null && (
                  <li>
                    ...
                    <br />
                    {stock.code} {stock.name} / 現物買い /{' '}
                    {shares.toLocaleString('ja-JP')}株 / 執行条件{' '}
                ...
                <li>
                  ...
                  <br />
                  現物売 / {shares.toLocaleString('ja-JP')}株 / 執行条件{' '}
                  <span className="font-medium">逆指値</span> /{' '}
                  <span className="font-medium tabular-nums">
                    {price(plan.stop)}円
                  </span>
